...
def training(beta, density, time_start, time_end, runs):
    ...
    out_csv = 'output-training/57600-61200-dqn'
    ...
    # 3. Tentuin area L1 L2 L3 L4 (cari tau caranya)
    ...
    # J = (P * (L2_max**2)) + (Q * L4_max) - (R * L1_min) 
    # return J
...
def validasi(beta, density, time_start, time_end, runs = 1):
    ...
    for run in range(1, runs+1): #diganti-ganti mau sekali running udah atau langsung banyak
        ...
        if done : #save nn weight if training done
                dqn_agent[agent].save('output-training\tesaja'+str(agent)+'__'+str(run)+'.h5')
