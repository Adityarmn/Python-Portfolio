...
class SumoEnvironment(gym.Env):
    ...
    Args:
        ...
        out_xml_name (Optional[str]): name of the .xml output with simulation results. If None, no output is generated
    ...
    def _get_per_agent_info(self):
        ...
        accumulated_waiting_time = [sum(self.traffic_signals[ts].get_accumulated_waiting_time_per_lane())
            for ts in self.ts_ids
        ]
    ...
    def save_xml(self, out_xml_name, episode):
        ...
        if out_xml_name is not None:
            ...
            df.to_xml(out_xml_name + f"_conn{self.label}_ep{episode}" + ".xml", index=False)
